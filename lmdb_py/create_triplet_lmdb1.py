...
def read_images(list_,new_height,new_width):#图像路径列表

	#list_=open(images_path).readlines()[i:i+batch]
	#
	# triplet a,p,b
	a_list=[]
	p_list=[]
	...
	for i in range(triplet_count):
		...
		if (len(a_data.shape)+len(p_data.shape)+len(n_data.shape)) == 3*3:
			...
			image_data=numpy.zeros((3*3,new_width,new_width),dtype=numpy.uint8)
