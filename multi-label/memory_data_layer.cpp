...
void MemoryDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  label_size_ =this->layer_param_.memory_data_param().label_size();//caffe.proto
...
void MemoryDataLayer<Dtype>::AddDatumVector(const vector<Datum>& datum_vector) {
  ...
  for (int item_id = 0; item_id < num; ++item_id) {
    for( int label_i=0;label_i<label_size_;label_i++)
      top_label[item_id*label_size_+label_i] = datum_vector[item_id].label(label_i);
  }
...
void MemoryDataLayer<Dtype>::AddMatVector(const vector<cv::Mat>& mat_vector,
  ...
  for (int item_id = 0; item_id < num; ++item_id) {
    for( int label_i=0;label_i<label_size_;label_i++)
      top_label[item_id*label_size_+label_i] = labels[item_id*label_size_+label_i];
  }
  // num_images == batch_size_
  Dtype* top_data = added_data_.mutable_cpu_data();
  Reset(top_data, top_label, num);
